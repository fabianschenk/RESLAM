...
public:
    ...
    void stitchDouble(MatXX &H, VecX &b, WindowedOptimizer const * const EF, bool usePrior, bool useDelta, int tid=0);

    template<int mode> void addPoint(EFPoint* p, WindowedOptimizer const * const ef, int tid=0);



    void stitchDoubleMT(IndexThreadReduce<Vec10>* red, MatXX &H, VecX &b, WindowedOptimizer const * const EF, bool usePrior, bool MT)
	{
        ...
        {
            I3D_LOG(i3d::detail) << "Allocating " << nframes[0]*HessianSize+CPARS << "x" << nframes[0]*HessianSize+CPARS;
	...
	EIGEN_ALIGN16 AccumulatorApprox* acc[NumThreads];
	int nres[NumThreads];

	template<int mode> void addPointsInternal(
            std::vector<EFPoint*>* points, WindowedOptimizer const * const ef,
