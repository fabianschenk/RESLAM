...
public:
         ...
         */
        // virtual void publishGraph(const std::map<uint64_t,Eigen::Vector2i, std::less<uint64_t>, Eigen::aligned_allocator<std::pair<uint64_t, Eigen::Vector2i> > > &connectivity) = 0;
        virtual void publishGraph(const ConnectivityMap &connectivity) = 0;
        virtual void publishEdgeGraph(const ConnectivityMap &connectivity) = 0;
        virtual void publishMargGraph(const ConnectivityMap &connectivity) = 0;
        virtual void publishCompleteTrajectory(const std::vector<std::unique_ptr<FrameHeader>>& frameHeaders) = 0;
        
        virtual void addGtTrajectory(const PoseVector& gtPoses) = 0;

        virtual void deleteTillFrameId(size_t frameId) = 0;
         ...
         */
        virtual void publishKeyframes(std::vector<FrameData*> &frames, const CameraMatrix& HCalib) = 0;
        virtual void updateKeyframePoses(std::vector<FrameHeader*> keyFrameHeaders) = 0;

        /**
         * Publishes only the newest keyframe.
         * This is only helpful when no windowed optimization is performed!
         */
        virtual void publishNewKeyframe(FrameData* frameData, const CameraMatrix& camMat) = 0;
         ...
         * Always called, no overhead if not used.
         */
        virtual void publishCamPose(const FrameHeader& frame,  const CameraMatrix& HCalib) = 0;
         ...
         * Always called, no overhead if not used.
         */
        virtual void pushLiveFrame(const FrameData * const image) = 0;
         ...
         */
        virtual void pushDepthImageFloat(MinimalImageF* image, FrameData* KF ) {}
